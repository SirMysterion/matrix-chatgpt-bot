...
import ChatGPTClient from '@sirmysterion/chatgpt-api';
...
import { DATA_PATH, KEYV_URL, OPENAI_API_KEY, MATRIX_HOMESERVER_URL, MATRIX_ACCESS_TOKEN, MATRIX_AUTOJOIN, MATRIX_BOT_PASSWORD, MATRIX_BOT_USERNAME, MATRIX_ENCRYPTION, MATRIX_THREADS, CHATGPT_CONTEXT, CHATGPT_MODEL, CHATGPT_ENDPOINT, KEYV_BOT_STORAGE, KEYV_BACKEND, CHATGPT_PROMPT_PREFIX, MATRIX_WELCOME } from './env.js'
...
async function main() {
  ...
  const clientOptions = {  // (Optional) Parameters as described in https://platform.openai.com/docs/api-reference/completions
    ...
    // reverseProxyUrl: CHATGPT_ENDPOINT,
    completionsUrl: CHATGPT_ENDPOINT,
  ...
  LogService.info("index", `Starting bot using Endpoint: ${CHATGPT_ENDPOINT}`);
